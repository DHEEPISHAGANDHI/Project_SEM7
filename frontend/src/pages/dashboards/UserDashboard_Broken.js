...
const UserDashboard = () => {
  ...
  // Additional state variables for the component
  ...
  const handleJoinCall = (request) => {
    if (request.meetingDetails?.meetingLink) {
      setActiveVideoConsultation(request);
      setShowVideoCall(true);
    } else {
      alert('Meeting link not available yet. Please wait for the lawyer to set up the meeting.');
    }
  };
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case 'pending':
        return { color: '#ffc107', text: 'Pending Review', icon: '⏳' };
      case 'approved':
        return { color: '#28a745', text: 'Approved - Schedule Now', icon: '✅' };
      case 'scheduled':
        return { color: '#17a2b8', text: 'Meeting Scheduled', icon: '📅' };
      case 'completed':
        return { color: '#6c757d', text: 'Consultation Completed', icon: '✔️' };
      case 'rejected':
        return { color: '#dc3545', text: 'Request Declined', icon: '❌' };
      case 'cancelled':
        return { color: '#6c757d', text: 'Cancelled', icon: '🚫' };
      default:
        return { color: '#6c757d', text: status, icon: '📋' };
    }
  };

  const renderConsultationRequests = () => (
    <div className={styles.consultationRequestsSection}>
      <div className={styles.sectionHeader}>
        <h3><TranslatableText text="My Consultation Requests" /></h3>
        <button 
          className={styles.newRequestButton}
          onClick={() => navigate('/free-consultation')}
        >
          <i className="fas fa-plus"></i>
          <TranslatableText text="New Request" />
        </button>
      </div>
      ...
      {userRequests.length === 0 ? (
        <div className={styles.emptyState}>
          ...
          <h4><TranslatableText text="No Consultation Requests Yet" /></h4>
          <p><TranslatableText text="Submit your first consultation request to get started with legal advice." /></p>
          <button 
              ...
              <div key={request.id} className={styles.requestCard}>
                ...
                <div className={styles.requestContent}>
                  <div className={styles.lawyerInfo}>
                    <h4>{request.lawyerName}</h4>
                    <p className={styles.legalIssue}>{request.legalIssue}</p>
                  </div>
                  ...
                  <div className={styles.requestDetails}>
                    ...
                    {request.meetingDetails?.scheduledTime && (
                      <div className={styles.detailItem}>
                        <i className="fas fa-video"></i>
                        <span>Meeting: {new Date(request.meetingDetails.scheduledTime).toLocaleString()}</span>
                      </div>
                    )}
                  </div>
                  ...
                  {request.status === 'scheduled' && (
                    <>
                      <button 
                        ...
                        disabled={!request.meetingDetails?.meetingLink}
                      >
                        <i className="fas fa-video"></i>
  ...
  // Handle schedule meeting action (from consultation card) - Updated function
  const handleScheduleMeetingOld = (consultation) => {
  ...
  // Handle join video call - Updated function
  const handleJoinCallOld = (consultation) => {
  ...
  const handleCancelRequestOld = (requestId) => {
  ...
  return (
    <div className={styles.userDashboard}>
      ...
      <div className={styles.sidebar}>
        ...
        <nav className={styles.sidebarNav}>
          ...
          >
            ...
            {userRequests.filter(r => r.status === 'approved' || r.status === 'scheduled').length > 0 && (
      ...
      <div className={styles.mainContent}>
        {activeTab === 'consultations' && renderConsultationRequests()}
